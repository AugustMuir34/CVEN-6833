...
write(t(cbind(xygrid1_sst,indexgrid_sst)),
      file="sst-locs.txt",
      ncol=3)
...
ggplot() +
  ...
  labs(title = "Eigen Spectrum SST",x = "Modes",y = "Frac. Var. explained")+
...
for (k in 1:4) {
  ...
  zfull[indexgrid_sst] = zsvd$u[,k]
  zmat = matrix(zfull, nrow=nrows_sst, ncol=ncols_sst)
  image.plot(xlong, ylat, zmat, ylim=range(-16,16))
  contour(xlong, ylat, zmat, ylim=range(-16,16), add=TRUE, nlev=3, lwd=2)
...
for(i in 1:4){
  ...
  zfull[indexgrid_sst]=zrot$loadings[,i]
  zmat = matrix(zfull,nrow=nx_sst,ncol=ny_sst)
  image.plot(xlong,ylat,zmat,ylim=range(-16,16),ann=FALSE)
...
### Correlate unrotated PCs with India Rainfall ####
nrows = 68
ncols = 65
ntime = 67
...
for (i in 1:4) {
  ...
  title(main = paste("Correlation of SST PC", i, "with India rainfall"))
  maps::map('world',wrap=c(0,360),add=TRUE,resolution=0,lwd=2)
  grid()
}